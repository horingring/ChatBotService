  ...
  makeChatList() {
    ...
    for (var i = 0; i < chatData.length; i++) {
      if (chatData[i].m_idx !== userData.m_idx) {
        chatList.push(
          <div className="chatNextBox" key={chatData[i].chatNum}>
            <div className="chatBoxContainerForOtherUser">
              ...
              <div className="chatNameAndContent_Wrapper">
                <div className="chatName">{chatData[i].user}</div>
                <div className="chatContent chatContent_left">
                  {chatData[i].content}
                </div>
              </div>
            </div>
        ...
        chatList.push(
          <div
            className="chatNextBox chatNextBox_right"
            key={chatData[i].chatNum}
          >
            <div className="chatContent chatContent_right">
              {chatData[i].content}
            </div>
          </div>
        );
      ...
      <div id="ChatBotChatting_Container">
        <div className="ChatBotChattingBox_Wrapper">
          <div //챗봇채팅창
            ...
            ref={(ref) => {
              this.refForScroll = ref;
            }} /*0724금, 경호, 스크롤 최하단 내리기(ref지정)*/
          >
            {this.makeChatList()}
          </div>
        </div>
        <div id="ChatBotInputBox_Container">
          {" "}
          {/* 챗봇입력 컨테이너 */}
          <form
            id="InputForm" //form
          ...
          >
            <div id="ChatBotInput_Box">
              {" "}
              {/* 입력창 박스 */}
              {/* <div>주문하기/취소하기/메뉴보기 버튼</div> 이 기능은 나중에 여유되면 구현*/}
              <textarea
                id="InputTextarea" //input 입력칸
                name="content"
                style={{ resize: "none" }}
